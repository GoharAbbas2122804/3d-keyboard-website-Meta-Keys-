...
/**
 * Props for `KeyBoardColorSelector`.
 */


type keycapTexture = (typeof KEYCAP_TEXTURES)[number];

export type KeyBoardColorSelectorProps =
  SliceComponentProps<Content.KeyBoardColorSelectorSlice>;

/**
 * Component for "KeyBoardColorSelector" Slices.
 */
const KeyBoardColorSelector: FC<KeyBoardColorSelectorProps> = ({ slice }) => {
  ...
  const backgroundText = KEYCAP_TEXTURES.find((t) => t.id === selectedTextureId)?.name || "";
  const repeatedBackgroundText = Array.from({ length: 10 }, () => backgroundText).join(" ");
  ...
  return (
    ...
    >
      ...
      <Bounded className="relative shrink-0" innerClassName="flex flex-col lg:flex-row ">
        ...
        <ul className="grid grow grid-cols-2 gap-3 rounded-3xl bg-white p-4  text-black shadow-lg sm:grid-cols-2 md:grid-cols-6 lg:grid-cols-3 xl:grid-cols-6">
...
export default KeyBoardColorSelector;
